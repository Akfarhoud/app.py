...
=======================================
A single-file Streamlit app designed for **non-professionals** to surface 
key People Analytics insights with minimal setup. Easy to deploy on Streamlit Cloud.
...
# PAGE & SIDEBAR
# ──────────────────────────────────────────────────────────────
st.set_page_config(page_title="People Analytics Lite", page_icon="📊", layout="wide")
st.sidebar.title("People Analytics – Lite v5")
st.sidebar.caption("Upload a CSV or use the built‑in demo data.")

# ──────────────────────────────────────────────────────────────
# DEMO DATA (synthetic HR table)
# ──────────────────────────────────────────────────────────────
@st.cache_data
def make_demo_data(n: int = 800) -> pd.DataFrame:
    ...
    salary = np.round(rng.normal(12000, 4000, n).clip(4000, 30000), -2)
...
# DATA SOURCE
# ──────────────────────────────────────────────────────────────
source = st.sidebar.radio("Data source", ["Use demo data", "Upload CSV"], index=0)
...
st.title("📊 People Analytics – Web Deploy Lite")
st.caption("Descriptive • KPIs • Engagement • Attrition • Pay Equity • Insights")

# ──────────────────────────────────────────────────────────────
# COLUMN MAPPER (Wizard)
# ──────────────────────────────────────────────────────────────
with st.expander("🔧 Column Mapper (set once)", expanded=True):
    cols = list(df.columns)
    def pick(label, hints):
        # try to guess column
        guess = next((c for c in cols if any(h in c.lower() for h in hints)), None)
        options = ["(None)"] + cols
        idx = 0 if guess is None else options.index(guess)
...
if hire_col != "(None)":
    df[hire_col] = to_datetime_safe(df[hire_col])
if exit_col != "(None)":
    df[exit_col] = to_datetime_safe(df[exit_col])

# Tenure (years)
if "TenureYears" not in df.columns:
    if hire_col != "(None)":
        today = pd.to_datetime("today").normalize()
        df["TenureYears"] = ((df[exit_col].fillna(today) - df[hire_col]) / np.timedelta64(1, 'Y')).round(2) if exit_col != "(None)" else ((today - df[hire_col]) / np.timedelta64(1, 'Y')).round(2)

# ──────────────────────────────────────────────────────────────
# TABS
# ──────────────────────────────────────────────────────────────
TAB1, TAB2, TAB3, TAB4, TAB5, TAB6, TAB7 = st.tabs([
    "Overview", "KPI Center", "Engagement", "Attrition", "Pay Equity", "What‑if", "Insights"
])
...
# ──────────────────────────────────────────────────────────────
with TAB2:
    st.subheader("KPI Center")
    if hire_col == "(None)":
        st.info("Set **Hire date** in Column Mapper to enable time-based KPIs.")
    else:
        min_d = pd.to_datetime(df[hire_col].min()).date()
        max_d = pd.to_datetime(df[exit_col].max()).date() if exit_col != "(None)" and df[exit_col].notna().any() else date.today()
        start, end = st.date_input("Date range", value=(min_d, max_d))
        if isinstance(start, tuple):
            start, end = start
        rng = pd.date_range(start=start, end=end, freq="M")
...
# ──────────────────────────────────────────────────────────────
with TAB3:
    st.subheader("Engagement health")
    eng_cols = eng_multi
    if eng_cols:
        eng = df[eng_cols].apply(pd.to_numeric, errors="coerce")
        score = eng.mean(axis=1)
        df["EngagementScore"] = score
        # Normalize to 0–100
        if score.max() <= 5:
            score_100 = (score - 1) / 4 * 100
        else:
            score_100 = score.clip(0, 100)
        df["EngagementPct"] = np.round(score_100, 1)
        df["EngagementFlag"] = pd.cut(df["EngagementPct"], bins=[-1, 60, 80, 101], labels=["❌ Low", "⚠️ Medium", "✅ High"])
...
# ──────────────────────────────────────────────────────────────
with TAB4:
    st.subheader("Attrition prediction (who might leave)")
    if attrition_flag_col == "(None)":
        st.info("Select **Attrition flag** in Column Mapper (1=left, 0=stayed). If you don't have a historical flag, you can still predict risk using current patterns, but accuracy will be limited.")
    # Features = all except target
    feature_cols = [c for c in df.columns if c != attrition_flag_col]
...
# ──────────────────────────────────────────────────────────────
with TAB5:
    st.subheader("Pay Equity Analyzer")
    if salary_col == "(None)":
        st.info("Select **Salary** in Column Mapper.")
    else:
        group_col = st.selectbox("Group by attribute", [c for c in [gender_col, dept_col] if c != "(None)"] + [c for c in df.select_dtypes(exclude=np.number).columns if c not in ["(None)", gender_col, dept_col]], index=0)
        if group_col:
            ref_group = st.selectbox("Reference group (baseline)", sorted(df[group_col].dropna().unique().tolist()))
            # Unadjusted gap (% difference in means vs ref)
            means = df.groupby(group_col)[salary_col].mean().sort_values()
            unadj = ((means / means.loc[ref_group] - 1) * 100).rename("Unadjusted % vs ref")
            st.write("Unadjusted pay vs reference:")
            st.dataframe(unadj.to_frame().round(2))
            st.plotly_chart(px.bar(unadj, title="Unadjusted pay gap (%)"), use_container_width=True)
...
# ──────────────────────────────────────────────────────────────
with TAB6:
    st.subheader("What‑if simulator (attrition risk)")
    if "_model" not in st.session_state:
        st.info("Train the attrition model first (TAB: Attrition).")
    else:
        model = st.session_state["_model"]
        scaler = st.session_state["_scaler"]
        colsX = st.session_state["_Xcols"]
        # Build X matrix again
        X_all = df[colsX].copy()
        for c in X_all.select_dtypes(exclude=np.number).columns:
...
# ──────────────────────────────────────────────────────────────
with TAB7:
    st.subheader("Plain-language insights & alerts")
    # Engagement verdict
    if "EngagementPct" in df.columns:
        avg_e = df["EngagementPct"].mean()
        low_e = (df["EngagementFlag"] == "❌ Low").mean() * 100
        if avg_e >= 80 and low_e < 10:
            st.success("Engagement looks **good** overall (avg ≥ 80% and <10% in Low).")
        elif avg_e < 60 or low_e >= 20:
            st.error("Engagement is **concerning** (avg < 60% or ≥20% in Low). Prioritise pulse checks and manager coaching.")
        else:
            st.warning("Engagement is **mixed**. Some teams need attention. Target Low group first.")
...
st.sidebar.markdown("---")
st.sidebar.caption("© 2025 – People Analytics Lite v5 | Built for easy cloud deployment")
